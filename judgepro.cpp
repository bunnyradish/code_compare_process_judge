...
void runCmd(char *args[],long timeLimit, long memoryLimit, char *in, char *out) {
    int newstdin = open(in,O_RDWR|O_CREAT,0777);
    int newstdout = open(out,O_RDWR|O_CREAT,0777);
...
int run(char *args[],int timeLimit, int memoryLimit, char *in, char *out, char *msg){
    pid_t pid = vfork();
    if(pid<0)
	...
	mmsg = fopen(msg,"w+");
	char stringx[1005];
        sprintf(stringx, "{\"status\":\"%d\",\"timeUsed\":\"%d\",\"memoryUsed\":\"%d\"}", rest.status, rest.timeUsed, rest.memoryUsed);
	printf("%s\n", stringx);
	fprintf(mmsg, stringx);
	//fwrite (stringx , sizeof(char), strlen(stringx), mmsg);
	//fputs(string, mmsg);
	fclose(mmsg);
    }
}
